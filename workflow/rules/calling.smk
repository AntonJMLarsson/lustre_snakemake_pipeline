rule get_concord_intervals_KNR:
    input: "results/KNR_bam_concord/{cell,[A-Z]+}.bam"
    output: temp("results/KNR_concord_bed/{cell,[A-Z]+}.bed")
...
rule get_concord_intervals_UNK:
    input:"results/UNK_bam_concord/{cell,[A-Z]+}.bam"
    output: temp("results/UNK_concord_bed/{cell,[A-Z]+}.bed")
    shell: "{config[bedtools]} bamtobed -i {input} | sort -k1,1 -k2,2n | {config[bedtools]} merge -c 5 -o mean -i - > {output}"

rule merge_concord_intervals_KNR:
    input: lambda wildcards:expand("results/KNR_concord_bed/{cell,[A-Z]+}.bed", cell=get_cells(wildcards))
...
rule merge_concord_intervals_UNK:
    input: lambda wildcards:expand("results/UNK_concord_bed/{cell,[A-Z]+}.bed", cell=get_cells(wildcards))
...
rule concat_concord_bam_KNR:
    input: lambda wildcards:expand("results/KNR_bam_concord/{cell,[A-Z]+}.bam", cell=get_cells(wildcards))
...
rule concat_concord_bam_UNK:
    input: lambda wildcards:expand("results/UNK_bam_concord/{cell,[A-Z]+}.bam", cell=get_cells(wildcards))
...
rule concat_discond_bam_KNR:
    input: lambda wildcards:expand("results/KNR_bam/{cell,[A-Z]+}.bam", cell=get_cells(wildcards))
...
rule concat_discond_bam_UNK:
    input: lambda wildcards:expand("results/UNK_bam/{cell,[A-Z]+}.bam", cell=get_cells(wildcards))
...
rule concord_filter_KNR:
     input: bam = "results/KNR_bam/{cell,[A-Z]+}.bam", bed = "results/KNR_concord_merged.filtered.bed"
     output: "results/KNR_bam_concord_filtered/{cell,[A-Z]+}.bam"
     shell: "{config[bedtools]} intersect -v -abam {input.bam} -b {input.bed} > {output}"

rule concord_filter_UNK:
     input: bam = "results/UNK_bam/{cell,[A-Z]+}.bam", bed = "results/UNK_concord_merged.filtered.bed"
     output: "results/UNK_bam_concord_filtered/{cell,[A-Z]+}.bam"
     shell: "{config[bedtools]} intersect -v -abam {input.bam} -b {input.bed} > {output}"

rule remove_supplemental_KNR:
    input: "results/KNR_bam_concord_filtered/{cell,[A-Z]+}.bam"
    output: "results/KNR_bam_concord_filtered_no_supp/{cell,[A-Z]+}.bam"
    shell: "{config[samtools]} view -o {output} -F 0x800 {input}"

rule remove_supplemental_UNK:
    input: "results/UNK_bam_concord_filtered/{cell,[A-Z]+}.bam"
    output: temp("results/UNK_bam_concord_filtered_no_supp/{cell,[A-Z]+}.bam")
    shell: "{config[samtools]} view -o {output} -F 0x800 {input}"

rule get_KNR_intervals:
    input: "results/KNR_bam_concord_filtered_no_supp/{cell,[A-Z]+}.bam"
    output: "results/KNR_intervals_bed/{cell,[A-Z]+}.bed"
...
rule get_UNK_intervals:
    input: "results/UNK_bam_concord_filtered_no_supp/{cell,[A-Z]+}.bam"
    output: "results/UNK_intervals_bed/{cell,[A-Z]+}.bed"
    shell: "{config[bedtools]} bamtobed -i {input} | sort -k1,1 -k2,2n | {config[bedtools]} merge -d 1000 -c 5 -o mean -i - > {output}"

rule merge_KNR_intervals:
    input: lambda wildcards:expand("results/KNR_intervals_bed/{cell,[A-Z]+}.bed", cell=get_cells(wildcards))
...
rule merge_UNK_intervals:
    input: lambda wildcards:expand("results/UNK_intervals_bed/{cell,[A-Z]+}.bed", cell=get_cells(wildcards))

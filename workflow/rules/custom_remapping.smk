...
for donor, donor_config in config['custom_references'].items():
    ...
    rule:
        ...
        input:
            reads="results/polyA_rich_fastqs/{sample,[A-Z]+}.fastq.gz", fa = "results/custom_references/{prefix}.fa".format(prefix = donor), faidx = "results/custom_references/{prefix}.fa.ann".format(prefix = donor)

...
for donor, donor_config in config['custom_references'].items():
    ...
    rule:
        ...
        input: lambda wildcards: expand("results/polyA_rich_mapped_custom_tagged_transformed/{sample}.bam", sample=get_cells_donor(wildcards, config['{}_specific_samples'.format(donor)], donor))

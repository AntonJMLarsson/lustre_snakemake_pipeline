...
for donor in config["custom_references"].keys():
    rule:
        ...
        shell: "python3 workflow/scripts/refine/prepare_insertion_table.py --prefix insertion_tables/{config[project]}_KNR_{params.donor} --name {config[project]} --regular {input.regular_stats} --remapped {input.remapped_stats} --read-mat {input.regular_read_mat} --read-mat-remapped {input.remapped_read_mat}"
    ...
    rule:
        ...
        shell: "python3 workflow/scripts/refine/prepare_insertion_table.py --prefix insertion_tables/{config[project]}_UNK_{params.donor} --name {config[project]} --regular {input.regular_stats} --remapped {input.remapped_stats} --read-mat {input.regular_read_mat} --read-mat-remapped {input.remapped_read_mat}"
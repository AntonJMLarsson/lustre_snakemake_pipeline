...
rule prepare_insertion_table_KNR:
    ...
    shell: "python3 ../scripts/refine/prepare_insertion_table.py --prefix insertion_tables/{config[project]}_KNR_{wildcards.donor} --name {config[project]} --regular {input.regular_stats} --remapped {input.remapped_stats} --read-mat {input.regular_read_mat} --read-mat-remapped {input.remapped_read_mat}"
...
rule prepare_insertion_table_UNK:
    ...
    shell: "python3 ../scripts/refine/prepare_insertion_table.py --prefix insertion_tables/{config[project]}_UNK_{wildcards.donor} --name {config[project]} --regular {input.regular_stats} --remapped {input.remapped_stats} --read-mat {input.regular_read_mat} --read-mat-remapped {input.remapped_read_mat}"
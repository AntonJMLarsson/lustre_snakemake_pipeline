rule count_cell_detection_UNK_remapped:
    ...
    shell: "mkdir -p results/remapped_stats && python3 workflow/scripts/count_cell_detection.py -bam {input.bam} -stats {input.stats} -sample {config[samplesheet]} -p results/remapped_stats/{config[project]}_UNK -ct BC -t 50"
...
rule count_cell_detection_KNR_remapped:
    ...
    shell: "mkdir -p results/remapped_stats && python3 workflow/scripts/count_cell_detection.py -bam {input.bam} -stats {input.stats} -sample {config[samplesheet]} -p results/remapped_stats/{config[project]}_KNR -ct BC -t 50"
